...
class RegNet(BaseNet):
    ...
    def layer_op(self, images, is_training, layer_id=-1):
        ...
        flow = tf.layers.dense(tf.layers.flatten(flow), 64, activation=tf.nn.relu)
        flow = tf.layers.dense(flow, 1, activation=tf.nn.sigmoid)
        #flow = tf.layers.dense(tf.layers.flatten(flow), 1, activation=tf.nn.sigmoid)
        print('\nAfter dense\n', flow)
