...
from niftynet.engine.windows_aggregator_classifier_multioutput import \
    ClassifierAggregatorMultioutput
...
class ClassificationApplication(BaseApplication):
    ...
    def __init__(self, net_param, action_param, action):
        ...
        tf.logging.info('starting classification multioutput application')
        self.action = action
        ...
        self.SUPPORTED_SAMPLING = {
            ...
            #'resize2': (self.initialise_resize_sampler,
            #            self.initialise_resize_sampler,
            #            self.initialise_resize2_aggregator),
    ...
    def initialise_aggregator(self):
        self.output_decoder = ClassifierAggregatorMultioutput(
            image_reader=self.readers[0],
            output_path=self.action_param.save_seg_dir)
    ...
    def connect_data_and_network(self,
        ...
        if self.is_training:
            ...
            #net_out1 = self.net(image, is_training=self.is_training)
            net_out1, net_out2, net_out3 = self.net(image, is_training=self.is_training)
            ...
            print('ClassificationApplication debug:')
            print('\nnet_out1', net_out1)
            print('\nnet_out2', net_out2)
            print('\nnet_out2', net_out3)
            ...
            image = tf.Print(tf.cast(image, tf.float32),
                                  [tf.reduce_mean(image), tf.reduce_max(image), tf.reduce_mean(tf.cast(tf.is_nan(image),tf.int32))], message='isnaninput')

    #net_out1 = self.net(image, is_training=self.is_training)
            net_out1, net_out2, net_out3 = self.net(image, is_training=self.is_training)
            print('net_out1.shape may need to be resized:', net_out1.shape)
            print('net_out2.shape may need to be resized:', net_out2.shape)
            print('net_out3.shape may need to be resized:', net_out3.shape)
            ...
            net_out1 = post_process_layer(net_out1)

            outputs_collector.add_to_collection(
                var=net_out1, name='window1',
                average_over_devices=False, collection=NETWORK_OUTPUT)
            outputs_collector.add_to_collection(
                var=net_out2, name='window2',
                average_over_devices=False, collection=NETWORK_OUTPUT)
            outputs_collector.add_to_collection(
                var=net_out3, name='window3',
                average_over_devices=False, collection=NETWORK_OUTPUT)
            outputs_collector.add_to_collection(
    ...
    def interpret_output(self, batch_output):
        if not self.is_training:
            return self.output_decoder.decode_batch(
                ...
                {'window1': batch_output['window1'],
                 'window2': batch_output['window2'],
                 'window3': batch_output['window3']},
