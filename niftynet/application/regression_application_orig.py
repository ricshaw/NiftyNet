...
class RegressionApplicationOrig(BaseApplication):
    REQUIRED_CONFIG_SECTION = "REGRESSION"

    def __init__(self, net_param, action_param, action):
        BaseApplication.__init__(self)
        tf.logging.info('starting regression application orig')
        self.action = action
    ...
    def connect_data_and_network(self,
        ...
        if self.is_training:
            ...
            tf.logging.info('DOING INFERENCE!')

            data_dict = switch_sampler(for_training=False)
            image = tf.cast(data_dict['image'], tf.float32)
            net_out = self.net(image, is_training=self.is_training)

            net_out = tf.Print(tf.cast(net_out, tf.float32), [tf.reduce_mean(image), tf.reduce_min(image), tf.reduce_max(image)], message='input')
            net_out = tf.Print(tf.cast(net_out, tf.float32), [tf.reduce_mean(net_out), tf.reduce_min(net_out), tf.reduce_max(net_out)], message='net_out')
            #outputs_collector.add_to_collection(var=tf.reduce_mean(net_out), name='net_out_mean', average_over_devices=False, collection=NETWORK_OUTPUT)
            ...
            net_out = tf.Print(tf.cast(net_out, tf.float32), [tf.reduce_mean(net_out), tf.reduce_min(net_out), tf.reduce_max(net_out)], message='net_out_post_proc')

            #mean_net_out = tf.reduce_mean(net_out)
            #outputs_collector.add_to_collection(
            #    var=[mean_net_out], name='post_proc_mean',
            #    average_over_devices=False, collection=CONSOLE)
