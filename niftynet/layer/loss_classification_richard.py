...
def cross_entropy_orig(prediction,
                  ground_truth):
    """
    ...
    #ground_truth = tf.one_hot(ground_truth,2)
...
def cross_entropy(prediction,
    ...
    #gt = tf.Print(tf.cast(ground_truth, tf.float32), [tf.cast(ground_truth, tf.int32)], message='check_gt')
    #print('\nPred:', prediction)
    #print('GT:', ground_truth)

    #loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=prediction, labels=ground_truth)
    #loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=prediction, labels=ground_truth)
    loss = tf.reduce_mean(-tf.reduce_sum( tf.multiply(ground_truth, tf.log(prediction + 1e-5)), axis=[1]))
