...
class RandomArtefactLayer(RandomisedLayer):
    ...
    def __init__(self, name='random_artefact'):
        ...
        self.artefact_prob = 0.2
        self.lambda_large = 1
        self.lambda_small = 5
        self.angles_stddev_large = 0.03*np.pi
        self.angles_stddev_small = 0.003*np.pi
        self.trans_stddev_large = 3
        self.trans_stddev_small = 0.3
    ...
    def printInfo(self, image_name):
        print('image name: ', image_name)
        print('num events: ', self._num_events)
        np.set_printoptions(precision=3)
        for i in range(self._num_events):
            print('time: %.3f' % self._event_times[0,i])
            print('angles: ', self._angles[:,i]*180/np.pi)
            print('trans: ', self._trans[:,i])
    ...
    def computeFourierTransform(self, image_3d):
        F = np.zeros(image_3d.shape, np.complex)
    ...
    def randomise_name(self, size=4, chars=string.ascii_uppercase + string.digits):
    ...
    def computeMasks(self, image_3d):
        ...
        masks.append(mask)
        for i in range(self._num_events-1):
    ...
    def computeWeights(self, masks, image_3d):
        ...
        if self.weighting == 'time':
            ...
            for i in range(self._num_events+1):
                mask = masks[i]
                r_masked = r_t[np.unravel_index(mask, r_t.shape, 'F')]
    ...
    def _randomise_artefact_3d(self):
        ...
        print('Artefact prob: %.2f' % prob)
        if prob<self.artefact_prob:
            self._apply_artefact = True
            ...
            self._image_name = self.randomise_name()
        else:
            self._apply_artefact = False


    def _apply_artefact_3d(self, image_3d, mod_i, interp_order=3):
        # Randomise each modality differently
        self.randomise()

        if self._apply_artefact == True:
            ...
            print('Applying artefact...')

            # Pad image
            pad_width = 20
            image_3d = np.pad(image_3d, pad_width, 'edge')

            # Normalise image values
            image_max = np.amax(image_3d)
            image_min = np.amin(image_3d)
            image_3d = image_3d - image_min

            # Generate a random image name
            image_name = self._image_name + '_' + str(mod_i)
            self.printInfo(image_name)
            #sio.savemat('before_' + image_name + '.mat', {'before_' + image_name: image_3d})	
            ...
            F_composite_reorder[np.unravel_index(mask, F_composite_reorder.shape, 'F')] = Fm_reorder[np.unravel_index(mask, Fm_reorder.shape, 'F')]
            ...
            for i in range(self._num_events):
                ...
                for j in range(np.int(event_length)):
                    ...
                    Fm = np.copy(F_transformed)
                    Fm_reorder = np.transpose(Fm, (1, 0, 2))
                    F_composite_reorder[np.unravel_index(mj, F_composite_reorder.shape, 'F')] = Fm_reorder[np.unravel_index(mj, Fm_reorder.shape, 'F')]

                # Apply end of mask
                mask_end = mask[event_length:,]
                Fm = np.copy(F_transformed)
                Fm_reorder = np.transpose(Fm, (1, 0, 2))
                F_composite_reorder[np.unravel_index(mask_end, F_composite_reorder.shape, 'F')] = Fm_reorder[np.unravel_index(mask_end, Fm_reorder.shape, 'F')]
            ...
            image_3d = np.abs(IF, dtype=np.float32)	

            # Undo image padding
            image_3d = image_3d[pad_width:-pad_width, pad_width:-pad_width, pad_width:-pad_width]
            #sio.savemat('after_' + image_name + '.mat', {'after_' + image_name: image_3d})
        else:
            print('No artefact applied!')
        return image_3d
        ...
        if isinstance(inputs, dict) and isinstance(interp_orders, dict):
            for (field, image) in inputs.items():
                ...
                for mod_i, interp_order in enumerate(interp_orders[field]):
                    if field != 'label':
                        print('Modality: ', mod_i)
                        if image.ndim == 4:
                            inputs[field][..., mod_i] = self._apply_artefact_3d(image[..., mod_i], mod_i, interp_order)
                        elif image.ndim == 5:
                            for t in range(image.shape[-2]):
                                inputs[field][..., t, mod_i] = self._apply_artefact_3d(image[..., t, mod_i], mod_i, interp_order)
                        else:
